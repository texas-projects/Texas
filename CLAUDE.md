...
```bash
pnpm test           # 后端单元测试（vitest run，CI 模式）
pnpm test:watch     # 后端测试监听模式（开发时使用）
pnpm test:coverage  # 后端测试 + 覆盖率报告
pnpm test:all       # 运行全部测试（后端 + 前端）
```
...
### 技术栈（v2 TypeScript 重构）

| 层       | Python v1            | TypeScript v2                         |
| -------- | -------------------- | ------------------------------------- |
| Web 框架 | FastAPI              | Fastify 5                             |
| ORM      | SQLAlchemy + Alembic | Prisma 6                              |
| 任务队列 | Dramatiq             | BullMQ                                |
| 配置校验 | Pydantic Settings    | TypeBox + 自定义 `loadConfig`         |
| 日志     | Structlog            | Pino                                  |
| LLM      | OpenAI Python SDK    | LangChain (`@langchain/openai`)       |
| 运行时   | Python 3.14          | Node.js ≥22.12.0                      |
| 包管理   | uv                   | pnpm 10.12.1                          |
| 测试     | pytest               | Vitest                                |
| Lint     | ruff + mypy          | ESLint + typescript-eslint + Prettier |
...
新 handler 使用 TypeScript 装饰器（注意：**PascalCase**，与 Python 版本的小写装饰器不同）：
...
├── models/      # （已弃用，ORM 模型迁移至 Prisma schema）
...
> ⚠️ `.env.example` 仍含 Python 旧版变量名（如 `postgresql+asyncpg://`、`CELERY_BROKER_URL`），以 `src/core/config.ts` 中的 `ConfigSchema` 为准。
